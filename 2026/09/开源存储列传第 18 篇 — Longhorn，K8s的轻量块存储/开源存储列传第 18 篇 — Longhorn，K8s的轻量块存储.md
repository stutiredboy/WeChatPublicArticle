...
2017 年 K8s 块存储的三个选项云盘省心，快照复制云平台包办绑死云厂商Ceph RBD功能全，规模大运维重，要专门团队本地盘性能最好，零成本机器一坏数据全丢空档：中小集群要的轻量块存储Longhorn 的定位就在这个空档里
...
Longhorn 时间线2017Rancher 起步K8s 轻量块存储2019.10进入 CNCFSandbox 项目2020SUSE 收购 Rancher项目归入 SUSE 体系2021.11CNCF 孵化Incubating1.12v2 引擎 GASPDK 用户态
...
一个 Longhorn 卷的结构应用 Pod块设备读写Longhorn 卷（PVC）引擎 Engine卷的前端，一个 Pod副本 1节点 A 上的 Pod副本 2节点 B 上的 Pod副本 3节点 C 上的 Pod引擎加副本，全部是 K8s 调度的 Pod
...
副本跨节点分布节点 A卷X 副本 1卷Y 副本 2节点 B卷X 副本 2卷Y 副本 3节点 C卷X 副本 3卷Y 引擎同一卷的副本打散在不同节点一台机器故障，每个卷最多损失一个副本
...
一次同步写入的路径应用写入引擎副本 1副本 2副本 3各节点磁盘数据落盘副本全部确认落盘，引擎才向前端返回成功任何一刻挂掉一个副本，已确认数据不丢

图：同步复制，全部副本确认后返回

某个副本掉线或者磁盘变慢，引擎会把它从链里摘除，剩余副本继续提供服务，卷的状态标记为降级。之后 Longhorn 会在其他节点上重建一个新副本，从健康副本把数据同步过去，恢复到设定的副本数。

副本故障与重建故障发生副本 1 正常副本 2 掉线降级运行卷继续读写剩余副本承担写入冗余度暂时下降重建副本新节点起新副本从健康副本同步数据恢复三副本重建按块级差异同步，不整盘重拷
...
三层数据保护副本同步复制到其他节点防单盘单机故障实时快照增量，留在集群内防误操作，可回滚分钟级定时备份推到集群外S3 兼容存储 / NFS防集群级灾难Recurring Job 定时执行：快照、备份、清理旧快照每个卷可以有自己的保护策略，按卷配置
...
Longhorn 组件总览控制面longhorn-manager每节点一个，操作 CRD，编排卷和副本longhorn-ui（Dashboard）Web 界面，运维入口CSI driver：对接 K8s 的标准接口数据面（每卷一组）engine 实例卷前端，处理读写replica 实例数据落盘，跨节点分布备份代理：推送快照到备份目标K8s 标准接口PVC / StorageClass / VolumeSnapshot，应用无感知，按标准 CSI 接入
...
v1 与 v2 数据路径对比v1 引擎（内核态路径）应用 → 内核块层→ 内核网络栈收发→ 中断、上下文切换→ 副本进程落盘高 IOPS 下 CPU 开销大v2 引擎（SPDK 用户态）应用 → NVMe 设备（NVMe-oF）→ 用户态 SPDK 驱动→ 轮询，无中断开销→ 直通 NVMe 盘落盘低延迟、高 IOPS、低 CPU 开销性能提升幅度为官方宣称，实际以自测为准
...
三类块存储的分工云盘公有云首选快照复制云平台包办不出云，多云不适用关键词：省心、绑定Longhorn自建、边缘、多云中小集群，装完即用不养存储团队关键词：轻量、够用Ceph RBD大规模统一存储池PB 级，多形态输出需要专门团队运维关键词：规模、全功能三者不互斥，一个集群里可以混用多个 StorageClass
