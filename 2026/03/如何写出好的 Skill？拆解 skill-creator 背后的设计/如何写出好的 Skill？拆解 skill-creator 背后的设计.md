---
title: 如何写出好的 Skill？拆解 skill-creator 背后的设计
date: 2026-03-09
source: https://mp.weixin.qq.com/s/4tBRIi0ybotOcYQpCwPAqg
images: 10
---
...
>   1. 1\. **Metadata \(name + description\)** \- Always in context \(~100 words\)
>   2. 2\. **SKILL.md body** \- When skill triggers \(<5k words\)
>   3. 3\. **Bundled resources** \- As needed by Codex \(Unlimited because scripts can be executed without reading into context window\)
> 
...
### 5.5 两条重要的避坑指南
  ...
  1. 1\. **避免深层嵌套引用** — 所有 reference 文件应该从 SKILL.md 直接链接，不要 A → B → C 式嵌套
  2. 2\. **长文件加目录** — 超过 100 行的 reference 文件要在顶部加 TOC，方便 Codex 预览全貌
...
两个问题：
  ...
  1. 1\. **做错了后果多严重？** — 越严重 → 越低自由度
  2. 2\. **有多少种"正确"的做法？** — 越多 → 越高自由度
...
首先明确**Skill是给 AI 写指令，而不是给人** ，Skill本质是：**用最少的 token，在正确的层级，给 AI 最精准的约束，让它在边界内自由发挥。**